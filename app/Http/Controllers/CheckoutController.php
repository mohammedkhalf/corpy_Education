...
class CheckoutController extends Controller
{
    public function home_checkout()
    {
        if (Auth::check()) 
        {
            $check_courses = Card::where('user_id' , Auth::user()->id)->get();
    ...
    public function purchase_courses (Request $request)
    {
        ...
        $payObj->save();
        $user_card = Card::where('user_id' , Auth::user()->id)->get();
        $total_item = 0;
        $k = 0;
        ...
        $ordObj->customer_id = Auth::user()->id;
